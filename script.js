...
let dracofeu=new Pokemon('Luxray',14,8,100,0.5);
let boulbi=new Pokemon('Arcanin',18,4,70,0.8);

while(luxray.hp >0 && arcanin.hp>0){
    luxray.attackPokemon(arcanin)
    if(arcanin.hp<=0){
        console.log(arcanin.name+" is dead !");
        break;
    }
    arcanin.attackPokemon(luxray)
    if(luxray.hp<=0){
        console.log(luxray.name+" is Dead !");
        break
    }
}